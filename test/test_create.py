...
CLI_PATH = TEST_DIR.parent / "app" / "bin" / "main.py"
...
'''
    ...
    def test_new_day_note(self):
        ...
        env = os.environ.copy()
        env["NOTES_TEST_DATE"] = TEST_DATE
        result = subprocess.run(["python3", str(CLI_PATH), "create"], cwd=NOTES_DIR, env=env, check=True, capture_output=True, text=True)
        expected = ''
        self.assertEqual(result.stdout, expected)
